...
abstract contract Pool {
    ...
    function getAllReceivers() public view returns (ReceiverProxyWeight[] memory weights) {
        ...
        address next = ReceiverWeightsImpl.ADDR_ROOT;
        while (true) {
            uint32 receiverWeight;
            uint32 proxyWeight;
            (receiver, next, receiverWeight, proxyWeight) = sender.receiverWeights.nextWeight(
                receiver,
                next
            );
    ...
    function getProxyWeights() public view returns (ReceiverWeight[] memory weights) {
        ...
        while (true) {
            uint32 weight;
            (receiver, next, weight, ) = proxy.receiverWeights.nextWeight(receiver, next);
            if (receiver == ReceiverWeightsImpl.ADDR_ROOT) break;
    ...
    function setDeltasFromNow(int128 amtPerWeightPerBlockDelta, uint64 blockEnd) internal {
        ...
        while (true) {
            uint32 receiverWeight;
            uint32 proxyWeight;
            (receiverAddr, next, receiverWeight, proxyWeight) = sender
                .receiverWeights
                .nextWeightPruning(receiverAddr, next);
    ...
    function updateProxyReceiversDeltaFromNow(
        ...
        while (true) {
            uint32 weight;
            (receiver, next, weight, ) = proxy.receiverWeights.nextWeightPruning(receiver, next);
            if (receiver == ReceiverWeightsImpl.ADDR_ROOT) break;
    ...
    function applyProxyDeltasOnReceivers(int8 multiplier) internal {
        ...
        while (true) {
            uint32 weight;
            (receiverAddr, next, weight, ) = proxy.receiverWeights.nextWeightPruning(
                receiverAddr,
                next
            );
            if (receiverAddr == ReceiverWeightsImpl.ADDR_ROOT) break;
        ...
        uint64 nextCycle = ProxyDeltasImpl.CYCLE_ROOT;
        ...
        while (true) {
            ...
            (cycle, nextCycle, thisCycleDelta, nextCycleDelta) = proxy
                .amtPerWeightDeltas
                .nextDeltaPruning(cycle, nextCycle, finishedCycle);
